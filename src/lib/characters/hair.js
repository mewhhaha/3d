...
export function tiedBun({color='#30271f',looseness=.65}={}) {
  return stage('tied-bun',ctx=>{
    ...
    const cap=fitSurface(ctx,{name:'HairFoundation',select:p=>p[1]>hairline(Math.atan2(p[0],p[2]-cz)),ease:.003,kind:'hair',color});
    // Scalp UVs keep fine texture detail wherever individual guides separate.
    const capPosition=cap.geometry.attributes.position,capUV=cap.geometry.attributes.uv;
    for(let i=0;i<capPosition.count;i++)capUV.setXY(i,(Math.atan2(capPosition.getX(i),capPosition.getZ(i)-cz)+Math.PI)/(Math.PI*2),(capPosition.getY(i)-1.60)/.125);
    cap.geometry.deleteAttribute('tangent');
    const groups=[[],[],[]],count=ctx.quality==='fine'?540:ctx.quality==='studio'?340:150;
    const guide=(a,i)=>Array.from({length:33},(_,j)=>{
      const t=j/32,side=a<0?-1:1,e=THREE.MathUtils.smoothstep(t,.58,1),start=hairline(a)-.003;
      const angle=a+(side*(Math.PI-.05)-a)*e+.10*Math.sin(Math.PI*t);
      const y=t<.60?start+(1.716-start)*Math.sin(t/.60*Math.PI/2):1.716-(1.716-1.695)*(t-.60)/.40;
    ...
    for(let i=0;i<55;i++){
      ...
      groups[0].push(sweep({points:guide(a,i),radii:[.0002,.0014,.0018,.0002],segments:40,sides:6,material:dark}));
    }
