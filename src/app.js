...
function resize() {
 ...
 camera.aspect=bounds.width/Math.max(1,bounds.height);camera.updateProjectionMatrix();render();
}
new ResizeObserver(resize).observe($('#viewport'));
function frame(nextView='perspective') {
 if(!root)return;view=nextView;
 const bounds=new THREE.Box3().setFromObject(root,true),center=bounds.getCenter(new THREE.Vector3()),radius=Math.max(bounds.getBoundingSphere(new THREE.Sphere()).radius,.01);
 ...
 const span=radius*3;ground.position.set(center.x,bounds.min.y-radius*.001,center.z);ground.scale.setScalar(span*10);grid.position.set(center.x,bounds.min.y-radius*.002,center.z);grid.scale.setScalar(span/10);lighting.fit(bounds);render();
}
...
function loadHash(){const hash=new URLSearchParams(location.hash.slice(1)),id=hash.get('model'),values=Object.fromEntries([...hash.entries()].filter(([key])=>key.startsWith('p.')).map(([key,value])=>[key.slice(2),value]));select(catalog.some(item=>item.model.id===id)?id:(catalog.some(item=>item.model.id==='reference-explorer')?'reference-explorer':catalog[0].model.id),values);}
...
$('#wireframe').onchange=event=>{scene.overrideMaterial=event.target.checked?wireMaterial:null;ground.visible=!event.target.checked;render();};$('#grid').onchange=event=>{grid.visible=event.target.checked;render();};$('#rotate').onchange=event=>{controls.autoRotate=event.target.checked;};$('#parameters').onsubmit=event=>event.preventDefault();canvas.addEventListener('keydown',event=>{if(event.key.toLowerCase()==='f')frame();});window.addEventListener('hashchange',()=>{try{loadHash();}catch(error){report(error);}});
