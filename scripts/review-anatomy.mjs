...
try{
 ...
 for(const component of ['hand','forearm'])for(const representation of ['sculpt','cage','baked']){
  ...
  report.variants[`${component}-${representation}`]={stats:await page.evaluate(()=>window.lab.stats()),rig:await page.evaluate(()=>window.lab.rigInfo()),bytes:bytes.length,sha256:createHash('sha256').update(bytes).digest('hex'),errors:validation.issues.numErrors,warnings:validation.issues.numWarnings};
  if(representation==='baked'){
   ...
   await page.evaluate(()=>{window.lab.frame('threequarter');window.lab.pose('Grasp',1.1);});await page.locator('canvas').screenshot({path:`${folder}/${component}-grasp.png`});
 ...
 for(const component of ['hand','forearm']){const hi=report.variants[`${component}-sculpt`].stats.triangles,lo=report.variants[`${component}-baked`].stats.triangles;assert.ok(hi/lo>10);report.variants[`${component}-baked`].triangleReduction=1-lo/hi;}
