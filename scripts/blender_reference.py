...
def build(source, destination):
    ...
    assert all(im.packed_file for im in bpy.data.images if im.type == 'IMAGE' and im.size[0] > 0)
    bpy.ops.render.render(write_still=True)
    report = dict(commit=__import__('os').environ.get('GITHUB_SHA','local'),blender=bpy.app.version_string,model=source.stem,meshes=len(meshes),bones=bone_count,uv_meshes=len(meshes),images=len(images),actions=actions,deformation_distance_m=movement,ik_controls=controls,native_reopened=True,studio_lights=3,textures_packed=True)
