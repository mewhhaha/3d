...
try{
 ...
 const validation=await validator.validateBytes(new Uint8Array(bytes),{maxIssues:0});await writeFile(`${folder}/validation.json`,JSON.stringify(validation,null,2));assert.equal(validation.issues.truncated,false,'Validation must finish');assert.equal(validation.issues.numErrors,0,JSON.stringify(validation.issues.messages));
