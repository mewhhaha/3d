...
import {xbotNames,xbotStance} from '../studies/xbot-pose.js';
...
async function render(name,options){
 ...
 for(const pose of ['neutral','confident']){
...
try{
 ...
 report.kinematics=xbotStance(controls);controls.reset();assert.deepEqual(controls.names.map(name=>controls.position(name)),rest);
 ...
 assert.deepEqual(await readFile(out+'/bind/imported-xbot.glb'),await readFile(out+'/confident/imported-xbot.glb'));report.exportIsolation='Byte-identical fresh bind exports from bind and posed previews';
