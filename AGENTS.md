...
### Guided-form continuation (latest)

Read `docs/guided-forms.md` and `docs/checkpoints/7f93a10.md` before editing the current `models/cyber-form-study.js`. The active scene keeps the pose/camera/reference annotations fixed while refining the actual supports. `surface-boundary.js` provides shared edges, partitioning and compact boundary/derivative constraints; these do not weld mesh topology. `surface-cache.js` is a measured approximation of expensive primary supports, not lossless geometry transfer. `contour-volume.js` lofts capped simple footprints and is used for boot soles.

Run `npm run review:forms` for the full fixed-camera study, visible-hair masks, support/cache checks, normal transfer and standalone component exports. Only the hair has high/low/baked modes; the whole android is not normal-baked or fully skinned. The visible mask improved, but the crown groove, doll-like face, broad armor and regular boots remain unacceptable as final likeness. Inspect clay and alternative views; a good hull or low mean normal error is not a visual acceptance gate. Keep the fringe maximum normal error visible. The videos are still metadata-only references, not watched demonstrations. Preserve prior recipes and push coherent checkpoints without force updates.
