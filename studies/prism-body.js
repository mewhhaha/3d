...
export default defineModel({id:'prism-body',title:'Connected ribcage, limb roots and joint cowls',parameters:{handStyle:{type:'select',options:['legacy','relaxed'],default:'legacy'},panelStyle:{type:'select',options:['broad','cutaway'],default:'broad'},massStyle:{type:'select',options:['profiled','sculpted'],default:'profiled'},gestureStyle:{type:'select',options:['fixed','counterpose'],default:'counterpose'},jointStyle:{type:'select',options:['open','housed'],default:'housed'}},build:p=>{
